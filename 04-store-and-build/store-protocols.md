...
**Default selection (`auto`).** When `--store auto`, Nix:
...
**Copy a path into a local file-backed cache:**
...
- [Binary caches](binary-caches.md) / [Substitutes and NAR info](substitutes-and-narinfo.md)
- [Binary cache hosting](../12-deployment-and-infra/binary-cache-hosting.md) — serving `file://` / HTTP / S3 caches
- [Remote builders](remote-builders.md) — build scheduling vs `--store`
- [Trusted users](../14-security-and-trust/trusted-users.md) / [Inter-machine trust](../14-security-and-trust/inter-machine-trust.md) — daemon trust vs store reachability
- [Machine mesh](../02-concepts/machine-mesh.md) — peer reachability for `ssh://` / `https://` stores
