---
status: index
---

# Flake Workflows

Typical flake output patterns.

## Contents

- [Packages, Apps, devShells](packages-apps-devShells.md) — Common flake outputs
- [nixosConfigurations](nixos-configurations.md) — NixOS systems via flakes
- [homeConfigurations](home-configurations.md) — Home Manager via flakes
- [Templates](templates.md) — flake init templates
- [checks and hydraJobs](checks-and-hydraJobs.md) — CI-oriented outputs
