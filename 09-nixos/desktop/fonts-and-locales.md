...
**Which locales are generated.** NixOS derives the locale archive from `i18n.defaultLocale`, `i18n.extraLocaleSettings`, and `i18n.extraLocales`. List additional locales in `i18n.extraLocales` (format `"nl_NL.UTF-8/UTF-8"`) or set `"all"` to install every glibc locale — each entry adds eval and build cost, so list only what you need. The older `i18n.supportedLocales` option is largely auto-computed and deprecated for manual edits; prefer `i18n.extraLocales` when adding locales beyond what the defaults imply.

**Keyboard and timezone (often confused with i18n).** Virtual-console layout is `console.keyMap` (default `"us"`) or, when `console.useXkbConfig = true`, derived from `services.xserver.xkb` settings. Graphical sessions use XKB options on X11 or compositor-specific settings on Wayland — not `i18n.*`. Clock and calendar timezone is `time.timeZone`, independent of locale strings.
