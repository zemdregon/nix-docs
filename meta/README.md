---
status: index
---

# Meta

Repo conventions and coverage tracking — not Nix subject matter.

Published site: [zemdregon.github.io/nix-docs](https://zemdregon.github.io/nix-docs/). Ground truth: [todo-coverage.md](todo-coverage.md). Cadence: [release-checklist.md](release-checklist.md).

## Contents

- [conventions.md](conventions.md) — Naming, stubs, cross-links, growth rules
- [site.md](site.md) — MkDocs Material + GitHub Pages build and deploy
- [research-method.md](research-method.md) — Pack → write → verify loop for filling articles
- [quality-checklist.md](quality-checklist.md) — Rubric before `status: complete`
- [sources.md](sources.md) — Living canonical upstream URL table
- [todo-coverage.md](todo-coverage.md) — Status ground truth, remaining work, audit hook
- [release-checklist.md](release-checklist.md) — Phase 5.1 operational refresh steps
- [rag/](rag/README.md) — Local Ollama embeddings + Chroma vector search over the wiki
- [examples/](examples/README.md) — Shared illustrative `.nix` fixtures (not a second wiki; walkthroughs live in [16-configuration-examples](../16-configuration-examples/README.md))

